...
reachablePositions board pos@(x,y) = filter (\p -> isOnBoard p && differentColor p) positions
...
reachablePositions' board (Just (Piece Black Pawn)) pos@(x,1) = filter (not . (isOponent Black board)) [(x,2),(x,3)]
                                                             ++ filter (isOponent Black board) (map (positionPlus pos) [(1,1),(-1,1)])
reachablePositions' board (Just (Piece White Pawn)) pos@(x,6) = filter (not . (isOponent White board)) [(x,5),(x,4)]
...
reachablePositions' board tile@(Just (Piece pc pt)) pos@(x,y) = case pt of
    ...
    Pawn   -> filter (isOponent pc board) (map (positionPlus (positionPlus pos (head (movement tile)))) [(1,0),(-1,0)]) -- Can only capture diagonally
           ++ filter (not . (isOponent pc board)) (map (positionPlus pos) (movement tile)) -- Can only move straight
...
iterateMovement _ _ [] = []
iterateMovement board pos (m:ms) = takeWhileInclusive movable (drop 1 (iterate (positionPlus m) pos)) ++ iterateMovement board pos ms
...
movement (Just (Piece _   Knight)) = [(a,b) | a <- q, b <- q, (a + b) `mod` 2 /= 0] where q = [-2,-1,1,2]
movement (Just (Piece _     Rook)) = straight
movement (Just (Piece _   Bishop)) = diagonal
movement (Just (Piece _    Queen)) = diagonal ++ straight
movement (Just (Piece _     King)) = [(x,y) | x <- [-1,0,1], y <- [-1,0,1], x /= 0 || y /= 0]
movement Nothing = []
